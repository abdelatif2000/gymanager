...
class Subscription extends Home_Controller {
    ...
    {   
        ...
        if($billing_type =='monthly'):
                $amount = $data['package']->monthly_price;
        ...
        $youCanPayUrl=$this->generateYouCanPay($payment_id,$ip_address,$amount);
      
        $data['init'] = $mercado['init'];
        $data['youCanPayUrl']=$youCanPayUrl;
    ...
    public function generateYouCanPay($order_id,$ip_address,$amount){
         //set the paramagers of youcanpay :
         YouCanPay::setIsSandboxMode(settings()->youcan_mode == "sandbox" ? true : false);
         $youCanPay = YouCanPay::instance()->useKeys(
            settings()->youcan_mode == "sandbox" ? settings()->youcansandboxPrivateKey : settings()->youcanPrivateKey,
            settings()->youcan_mode == "sandbox" ? settings()->youcansandboxPublicKey : settings()->youcanPublicKey
         );
         $token = $youCanPay->token->create(
             ...
             $ip_address,
             base_url('admin/subscription/payment_success/'.$this->session->userdata('billing_type').'/'.$this->session->userdata('package_id').'/'.$this->session->userdata('payment_id').'/mercadopago'),
             base_url('admin/subscription/payment_cancel/'.$this->session->userdata('billing_type').'/'.$this->session->userdata('package_id').'/'.$this->session->userdata('payment_id').'/mercadopago'),
         );
